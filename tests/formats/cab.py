...
def run_tests(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    test_suite = CabTestSuite(config)
...
class CabTestSuite:
    ...
    def test_basic_compression(self) -> Dict[str, Any]:
        ...
        extract_dir = None
        try:
            test_file = self.temp_dir / 'cab_basic.txt'
            test_file.write_text('cab basic test')
            archive_file = self.temp_dir / 'cab_basic.cab'
            result = self._run_command([str(self.sevenz_exec), 'a', '-tcab', str(archive_file), str(test_file)])
            ...
            extract_dir = self.temp_dir / 'cab_extract'
    ...
    def test_integrity(self) -> Dict[str, Any]:
        ...
        archive_file = None
        try:
            test_file = self.temp_dir / 'cab_integrity.txt'
            test_file.write_text('cab integrity test')
            archive_file = self.temp_dir / 'cab_integrity.cab'
            result = self._run_command([str(self.sevenz_exec), 'a', '-tcab', str(archive_file), str(test_file)])
            if not result['success'] or not archive_file.exists():
                return {'status': 'failed', 'duration': time.time() - start_time, 'error': result.get('stderr', '')}
    ...
    def test_extraction(self) -> Dict[str, Any]:
        ...
        try:
            ...
            archives = list(reference_dir.glob('*.cab'))
            if not archives:
                return {'status': 'skipped', 'duration': time.time() - start_time, 'reason': 'no_reference_cab'}

            archive_file = archives[0]
            extract_dir = self.temp_dir / 'cab_reference_extract'
            extract_dir.mkdir(exist_ok=True)
