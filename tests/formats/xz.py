...
class XzTestSuite:
    ...
    def test_basic_compression(self) -> Dict[str, Any]:
        ...
        extract_dir = None
        try:
            test_file = self.temp_dir / 'xz_basic.txt'
            test_file.write_text('xz basic test')
            archive_file = self.temp_dir / 'xz_basic.xz'
            result = self._run_command([str(self.sevenz_exec), 'a', '-txz', str(archive_file), str(test_file)])
            ...
            extract_dir = self.temp_dir / 'xz_extract'
    ...
    def test_integrity(self) -> Dict[str, Any]:
        ...
        archive_file = None
        try:
            test_file = self.temp_dir / 'xz_integrity.txt'
            test_file.write_text('xz integrity test')
            archive_file = self.temp_dir / 'xz_integrity.xz'
            result = self._run_command([str(self.sevenz_exec), 'a', '-txz', str(archive_file), str(test_file)])
            if not result['success'] or not archive_file.exists():
                return {'status': 'failed', 'duration': time.time() - start_time, 'error': result.get('stderr', '')}
    ...
    def test_extraction(self) -> Dict[str, Any]:
        ...
        try:
            ...
            archives = list(reference_dir.glob('*.xz'))
            if not archives:
                return {'status': 'skipped', 'duration': time.time() - start_time, 'reason': 'no_reference_xz'}

            archive_file = archives[0]
            extract_dir = self.temp_dir / 'xz_reference_extract'
            extract_dir.mkdir(exist_ok=True)
