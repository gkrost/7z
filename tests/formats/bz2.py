...
def run_tests(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    test_suite = Bzip2TestSuite(config)
...
class Bzip2TestSuite:
    ...
    def test_basic_compression(self) -> Dict[str, Any]:
        ...
        extract_dir = None
        try:
            test_file = self.temp_dir / 'bzip2_basic.txt'
            test_file.write_text('bzip2 basic test')
            archive_file = self.temp_dir / 'bzip2_basic.bz2'
            result = self._run_command([str(self.sevenz_exec), 'a', '-tbzip2', str(archive_file), str(test_file)])
            ...
            extract_dir = self.temp_dir / 'bzip2_extract'
    ...
    def test_integrity(self) -> Dict[str, Any]:
        ...
        archive_file = None
        try:
            test_file = self.temp_dir / 'bzip2_integrity.txt'
            test_file.write_text('bzip2 integrity test')
            archive_file = self.temp_dir / 'bzip2_integrity.bz2'
            result = self._run_command([str(self.sevenz_exec), 'a', '-tbzip2', str(archive_file), str(test_file)])
            if not result['success'] or not archive_file.exists():
                return {'status': 'failed', 'duration': time.time() - start_time, 'error': result.get('stderr', '')}
    ...
    def test_extraction(self) -> Dict[str, Any]:
        ...
        try:
            ...
            archives = list(reference_dir.glob('*.bz2'))
            if not archives:
                return {'status': 'skipped', 'duration': time.time() - start_time, 'reason': 'no_reference_bz2'}

            archive_file = archives[0]
            extract_dir = self.temp_dir / 'bzip2_reference_extract'
            extract_dir.mkdir(exist_ok=True)
